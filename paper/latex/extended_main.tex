...
$g_\psi$ & Architecture & Module & 2-layer Projector MLP acting as an Information Bottleneck: $\mathbb{R}^{384} \to \mathbb{R}^{1024} \to \mathbb{R}^{128}$. \\
...
$\overline{\text{Dice}}_{\text{macro}}$ & $[0, 1]$ & Metric & Unstratified macro-average Dice evaluated across all $N_{\text{test}}$ test slices. \\
$\text{Dice}_{\text{tumor}}$ & $[0, 1]$ & Metric & Stratified macro-average Dice evaluated exclusively over tumor-bearing slices $\mathcal{B}_{\text{tumor}}$. \\
...
$\alpha$ & $[0, 1]$ & Scalar & Ratio of tumor-free true-negative slices in evaluation split: $\alpha = |\mathcal{B}_0| / N_{\text{test}}$ ($\alpha \approx 0.4648$). \\
...
$\text{Dice}_{\text{3D}}^{(k)}$ & $[0, 1]$ & Metric & Official BraTS 3D patient volumetric Dice evaluated across reconstructed volume $\mathbf{V}_P^{(k)}$ and $\mathbf{V}_Y^{(k)}$. \\
...
$\mathbf{N}$ & $\mathbb{R}^{H \times W}$ & Tensor & Additive zero-mean Gaussian noise tensor simulating low-field (1.5T) acquisition noise. \\
...
$\tilde{X}_{\text{noise}}$ & $\mathbb{R}^{B \times C \times H \times W}$ & Tensor & High-SNR asymptotic Rician noise perturbed slice batch: $(X + \mathbf{N}) \odot \mathbb{I}(X \neq 0)$. \\

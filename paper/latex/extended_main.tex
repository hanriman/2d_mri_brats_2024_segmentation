...
\paragraph{Theoretical Mechanics of Latent Effective Rank (Table~\ref{tab:benchmark_results_ext})}
An important empirical observation in Table~\ref{tab:benchmark_results_ext} is that VisReg JEPA achieves the highest Effective Feature Rank ($\textbf{26.38}$), nearly $3\times$ higher than standard I-JEPA ($9.02$) and higher than SigReg JEPA ($11.88$), while SigReg JEPA achieves the top self-supervised downstream segmentation performance ($0.8271$ vs $0.8177$ and $0.8147$ Dice).

This dimensional expansion in VisReg is directly attributable to the complementary action of its dual regularization objectives: the variance penalty $\mathcal{L}_{\text{var}}$ explicitly forces the coordinate-wise empirical standard deviation above $\gamma = 1.0$, while the Sliced-Wasserstein Distance $\mathcal{L}_{\text{SWD}}$ continuously penalizes deviations from the standard Gaussian quantile distribution across $M = 256$ random 1D projections. By penalizing both axis-aligned variance shrinkage and multi-directional projection collapse, VisReg forces the singular value spectrum to disperse across more orthogonal dimensions. In contrast, SigReg JEPA regularizes representations using the joint Epps--Pulley characteristic function test without explicit 1D quantile sorting, preserving a well-calibrated semantic geometry ($\text{Rank}_{\text{eff}} = 11.88$) that yields top downstream segmentation transfer ($0.8271$ Dice). Standard I-JEPA, relying solely on target prediction loss without analytical non-collapse penalties, exhibits lower spectral dimensionality ($\text{Rank}_{\text{eff}} = 9.02$). Total VisReg loss is:
\begin{equation}
...
\begin{enumerate}
    ...
    In the full-data regime ($100\%$ labels / $6.3\text{k}$ slices under $p_{\text{drop}}=0.25$), nnU-Net achieves top boundary sharpness ($\text{HD95} = 40.71\,\text{px}$) because its horizontal concatenation skip connections transfer high-frequency spatial gradients directly from early convolutional stages to the decoder. However, SigReg JEPA closely trails nnU-Net's boundary precision ($\text{HD95} = 41.67\,\text{px}$, within $0.96\,\text{px}$) and segmentation accuracy ($0.8271$ vs $0.8366$ Dice), while operating with a pure bottleneck architecture that provides a $2.39\times$ inference latency reduction ($6.94\,\text{ms}$ vs $16.57\,\text{ms}$) and a $2.06\times$ training speedup ($39.23\,\text{s}$ vs $80.69\,\text{s/epoch}$).
    Furthermore, under extreme label scarcity ($1\%$ labels / $63$ slices), skip connections become a critical failure mode: early convolutional layers lack semantic abstraction and overfit to high-frequency background noise, causing nnU-Net's performance to degrade to $0.3847$ Dice (and UNet to $0.0421$). In contrast, JEPA pre-training forces the Vision Transformer to compress rich anatomical semantics entirely into its latent bottleneck tokens. Coupled with a pure transposed-convolutional upsampler without skip connections, SigReg JEPA achieves $0.5933$ Dice—a $+54.2\%$ relative improvement over nnU-Net.
    
    \item \textbf{Heuristic-Free Non-Collapse Mechanics via Random Sketching}:
    Standard I-JEPA relies on an EMA teacher target encoder, which consumes additional VRAM and requires delicate momentum scheduling. By mapping representations through a 2-layer Projector MLP and testing 1D projections against standard normal quantiles, SigReg JEPA and VisReg JEPA completely eliminate EMA target encoders. VisReg JEPA expands representation rank to $\text{Rank}_{\text{eff}} = 26.38$ (nearly $3\times$ higher than standard I-JEPA's $9.02$), while SigReg JEPA balances spectral entropy ($\text{Rank}_{\text{eff}} = 11.88$) with top self-supervised segmentation transfer ($0.8271$ Dice), while training $>2.0\times$ faster per epoch than nnU-Net ($39.2\,\text{s}$ vs $80.7\,\text{s}$).

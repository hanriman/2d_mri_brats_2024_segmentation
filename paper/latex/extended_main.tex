...
$M = 256$ random projection vectors $u_m$ are sampled uniformly from the unit hypersphere: $u \sim \mathcal{N}(0, \mathbf{I}_{d_p}), u_m = u / \|u\|_2 \in \mathbb{S}^{d_p - 1}$. 1D slices are formed by matrix multiplication: $p_m = Z_{\text{proj}} u_m \in \mathbb{R}^N$ ($N = B \cdot N_{\text{ctx}}$).
...
\begin{equation}
    \tilde{p}_{im} = \frac{p_{im} - \mu_m}{\sigma_m + \epsilon}, \qquad \mu_m = \frac{1}{N} \sum_{i=1}^N p_{im}, \quad \sigma_m = \sqrt{\frac{1}{N} \sum_{i=1}^N (p_{im} - \mu_m)^2 + \epsilon}
\end{equation}
...
\begin{equation}
    \mathcal{L}_{\text{SWD}}(Z_{\text{proj}}) = \frac{1}{M} \sum_{m=1}^M \frac{1}{N} \sum_{i=1}^N \left| \tilde{p}_{(i) m} - \Phi^{-1}\left(\frac{i - 0.5}{N}\right) \right|
\end{equation}
where the target analytical quantiles are evaluated using the inverse error function:
\begin{equation}
    \Phi^{-1}\left(\frac{i - 0.5}{N}\right) = \sqrt{2} \cdot \text{erf}^{-1}\left(2 \left(\frac{i - 0.5}{N}\right) - 1.0\right)
\end{equation}

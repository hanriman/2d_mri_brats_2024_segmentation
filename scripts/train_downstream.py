...
def main():
    ...
    pretrained_ckpt = CHECKPOINTS_DIR / f"best_{args.model_type}.pt"
    if pretrained_ckpt.exists():
        logger.info(f"Loading pre-trained {args.model_type} encoder from {pretrained_ckpt.name}...")
        ckpt = torch.load(pretrained_ckpt, map_location=device)
        model.load_pretrained_encoder(ckpt["context_encoder_state_dict"])
    else:
        logger.warning(f"Pre-trained checkpoint {pretrained_ckpt.name} not found! Initializing with random weights.")

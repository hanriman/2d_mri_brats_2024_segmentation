...
def main():
    ...
    val_loss_fn = CombinedDiceBCELoss(dice_weight=1.0, bce_weight=1.0)  # For eval mode (single-resolution output)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                ...
                with torch.amp.autocast(device_type=device.type, enabled=use_amp):
                    logits = model(images)
                    loss = val_loss_fn(logits, labels)
        ...
        if avg_val_dice > best_dice:
            ...
            ckpt_path = ckpt_dir / "best_nnunet.pt"
            torch.save({
                "epoch": epoch,

...
class IJEPALoss(nn.Module):
    ...
    def forward(
        ...
        if len(predictions) == 0:
            # Return a zero tensor that supports .backward() to avoid crashing the training loop
        ...
        for pred, tgt in zip(predictions, targets):
            ...
            if self.loss_type in ("l1", "smooth_l1"):
                block_loss = F.smooth_l1_loss(pred_norm, tgt_norm)
            else:
